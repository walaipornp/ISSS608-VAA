...
```
```{r}
p_father <- ggplot(student_viz_data, 
...
tab_father <- table(student_viz_data$Father_Edu,
                    student_viz_data$GPA)

# Run Chi-square test
chisq.test(tab_father)
...
father_plot_data <- student_viz_data |>
  count(Father_Edu, GPA) |>
  group_by(Father_Edu) |>
  mutate(perc = n/sum(n))

mother_plot_data <- student_viz_data |>
  count(Mother_Edu, GPA) |>
  group_by(Mother_Edu) |>
  mutate(perc = n/sum(n))
...
```{r}
p_father_chi <- ggplot(father_plot_data,
       aes(x = factor(Father_Edu),
           y = perc,
           fill = factor(GPA))) +
...
```{r}
p_mother <- ggplot(student_viz_data, 
